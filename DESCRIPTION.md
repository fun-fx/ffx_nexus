...
- Dashboard expansion + embedded serving: the console now serves the built SPA directly from the Go binary (`web/embed.go`), so the dashboard ships in the container with no separate static host. New panels surface **model routing** (per-model effective quality/cost/latency, was API-only), **eval scores** (per-evaluator/metric avg score + pass rate from `eval_scores`), plus **cache-hit rate** and **guardrail-event** cards. Semantic-cache hits and guardrail actions are now persisted to ClickHouse (migration `003_dashboard.sql`) so they aggregate over a window instead of being live-feed-only. Trace rows show cache/guardrail flags. (Also fixed a latent `UInt64→int64` scan bug in the stats query.)
- Cozystack/Talos IaC: managed ClickHouse/Postgres/Redis Custom Resources, a tenant manifest, a Helm values override, and a deployment runbook make the production deployment reproducible from source. ClickHouse Keeper is disabled (Nexus uses only non-replicated MergeTree, so a single-node Keeper would crash-loop on quorum). This on-prem IaC lives in a separate private operations repo.
...
- Offline regression eval batch (`cmd/nexus-evalbatch`): runs a JSONL dataset through the Python eval service (no sampling), aggregates per-metric scores, and fails CI when scores regress beyond a tolerance versus a stored baseline. Optionally generates missing outputs via any OpenAI-compatible endpoint. A `-evaluator heuristic` mode scores with the built-in LLM-free heuristics for a fully hermetic, deterministic run.
- Eval regression CI gate: a dedicated GitHub Actions job runs `nexus-evalbatch -evaluator heuristic` over the committed dataset against `datasets/regression_baseline.json` on every PR, failing the build on any quality regression — no provider keys or eval service required.
...
- Prod rollout: ClickHouse `medium` preset, memory-bounded stats queries, manifests for Ollama + eval sidecar, and a prod values override enable routing, guardrails, judge, eval service, and semantic cache. (On-prem manifests live in a separate private operations repo.)
- Phase 5 self-host profile: `values-full.yaml` — see [docs/packaging.md](docs/packaging.md).
