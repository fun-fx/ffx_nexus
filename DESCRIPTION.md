...
- Structured-output self-correction (hot path, non-streaming): when the schema guardrail rejects a JSON response, the gateway retries the model with a correction prompt up to N times before falling back to `422`; corrections are traced as `self_corrected:N`.
