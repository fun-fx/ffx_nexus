...
- Cozystack/Talos IaC (`deploy/cozystack`): the managed ClickHouse/Postgres/Redis Custom Resources, a tenant manifest, a Helm values override, and a deployment runbook — the production deployment is now reproducible from source. ClickHouse Keeper is disabled (Nexus uses only non-replicated MergeTree, so a single-node Keeper would crash-loop on quorum).
