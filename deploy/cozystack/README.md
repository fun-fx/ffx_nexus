...
- A container registry reachable from the Talos nodes. This setup uses the
  in-cluster Harbor at `harbor.<node-ip>.nip.io` (Talos containerd cannot
...
```bash
docker build -t harbor.<node-ip>.nip.io/ffx/nexus:0.1.0 .
docker push harbor.<node-ip>.nip.io/ffx/nexus:0.1.0
```
...
kubectl -n tenant-nexus create secret docker-registry harbor \
  --docker-server=harbor.<node-ip>.nip.io \
