...
describe("MCPLogs", () => {
  ...
  it("renders an empty table when the API encodes a nil slice as null", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string) => {
        if (url.includes("/api/mcp-logs/stats")) {
        ...
        if (url.includes("/api/mcp-logs/filterdata")) {
          return new Response(JSON.stringify({ tool_names: null, server_labels: null }), {
            status: 200,
          });
        }
        if (url.includes("/api/mcp-logs")) {
          return new Response(JSON.stringify({ items: null, next_cursor: { before: "", since: "" } }), {
            status: 200,
          });
        }
        return new Response("{}", { status: 404 });
