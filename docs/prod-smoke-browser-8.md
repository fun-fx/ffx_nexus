...
대상: `https://console.tail7d361a.ts.net` (Tailscale tailscale 통해 접근)
SSO IdP: `https://keycloak.tail7d361a.ts.net` (Keycloak)
...
1. 시크릿 창으로 `https://console.tail7d361a.ts.net` 접속
...
2. Keycloak 로그인 페이지로 302 redirect (URL: `https://keycloak.tail7d361a.ts.net/realms/.../protocol/openid-connect/auth?...`)
...
3. `https://nexus.tail7d361a.ts.net/v1/chat/completions` 에 bearer token 으로 호출
