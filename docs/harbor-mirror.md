...
2. Mirrors the same image to `harbor.<tailnet>.ts.net/ffx/nexus:$TAG` and `:$MAJOR.$MINOR` (the production registry the Talos kubelet pulls from).
...
docker login harbor.<tailnet>.ts.net   # operator-level credentials
...
- `harbor.<tailnet>.ts.net` — Tailscale-only, connected to the production kubelet. Required because the Talos node certificates trust the Tailscale CA but **not** the public-internet CA chains for arbitrary hosts.
