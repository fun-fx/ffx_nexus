...
- Harbor가 두 주소로 노출: LAN `harbor.192.168.0.101.nip.io` / Tailscale
  `harbor.tail7d361a.ts.net`. 클러스터는 ts.net으로 pull.
- helm chart: `deploy/helm/nexus/` (Chart `0.3.3`, values.yaml 존재).
- prod values: `deploy/cozystack/values-prod.yaml`
  (`image.repository=harbor.tail7d361a.ts.net/ffx/nexus`, `tag=main-latest`, `pullPolicy=Always`).
