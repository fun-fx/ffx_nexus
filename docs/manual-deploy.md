...
- 로컬 Mac에 Tailscale 설치 + `tail7d361a.ts.net` 망에 로그인
...
KUBECONFIG=~/kubeconfig/prod.yaml kubectl -n tenant-nexus create secret docker-registry harbor \
  --docker-server=harbor.tail7d361a.ts.net \
...
# https://192.168.0.101:6443 또는 Tailscale MagicDNS 이름
```
...
# 또는 인-클러스터에서 pull 시도
KUBECONFIG=~/kubeconfig/prod.yaml kubectl -n tenant-nexus run test-pull --rm -it --image=harbor.tail7d361a.ts.net/ffx/nexus:$TAG --restart=Never --command -- sh
```
