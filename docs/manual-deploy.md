...
**해결**: `values-prod.yaml`의 `image.repository`가 `harbor.<node-ip>.nip.io/ffx/nexus`인지 확인. 노드 containerd는 이 LAN 호스트를 신뢰한다(검증: 같은 호스트로 fun-fx/ffx_web이 정상 배포/pull 중). CD 파이프라인도 동일 호스트로 push/pull한다.
