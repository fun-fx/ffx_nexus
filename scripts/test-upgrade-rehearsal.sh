...
helm upgrade "${RELEASE}" "${CHART_PATH}" \
  ...
  --set networkPolicy.providerEgress.proxy.host="nexus-test-egress-proxy-mock.nexus-test-proxy.svc.cluster.local" \
  --set networkPolicy.providerEgress.proxy.port=3128 \
  --set networkPolicy.providerEgress.proxy.namespace="nexus-test-proxy" \
