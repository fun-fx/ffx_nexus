...
if [[ $SKIP_BUILD -eq 0 ]]; then
  ...
  TMP_KANIKO=$(mktemp)
  sed "s|--destination=harbor\\.[^:]*:.*|--destination=harbor.192.168.0.101.nip.io/ffx/nexus:$CURRENT_TAG|" \
...
if [[ $DRY_RUN -eq 0 ]]; then
  ...
  GW_URL="${GW_URL:-https://nexus.tail7d361a.ts.net}"
  CON_URL="${CON_URL:-https://console.tail7d361a.ts.net}"
